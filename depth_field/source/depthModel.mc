...
//!   long the baseline itself is the most likely culprit, so a watchdog starts
//!   over from the current pressure — otherwise a bad sample that made the
//!   watch look permanently submerged would have no way out.
//!
//! The window is two half-window buckets rather than a ring of samples: the
//! effective window is therefore somewhere between one and two bucket lengths,
//! which is all the precision this needs and costs two floats.
//!
//! Note the thresholds below are engineering estimates, not measured values.
//! They want revisiting once the barometer's actual behaviour under water has
//! been checked on a real device (see the open question in plan.md).
class DepthModel {
