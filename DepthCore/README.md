...
```xml
<iq:barrels>
  <iq:depends name="DepthCore" version="2.3.6"/>
</iq:barrels>
```
...
The version in `manifest.xml` and the `<iq:depends>` version in each app must match. Both are `2.3.6`. Bump them together.
