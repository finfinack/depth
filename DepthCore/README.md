...
The depth model shared by the app and the four data fields, packaged as a [Monkey Barrel](https://developer.garmin.com/connect-iq/core-topics/shareable-libraries/).
...
```xml
<iq:barrels>
  <iq:depends name="DepthCore" version="2.3.1"/>
</iq:barrels>
```
...
The version in `manifest.xml` and the `<iq:depends>` version in each app must match. Both are `2.3.1`. Bump them together.
