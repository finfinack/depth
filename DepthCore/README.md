...
```xml
<iq:barrels>
  <iq:depends name="DepthCore" version="2.3.5"/>
</iq:barrels>
```
...
The version in `manifest.xml` and the `<iq:depends>` version in each app must match. Both are `2.3.5`. Bump them together.
